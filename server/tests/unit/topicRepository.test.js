...
describe("fetchTopicPapers", () => {
    ...
    it("Fetches the top 5 papers associated with a particular topic", async () => {
        pool.query.mockResolvedValue({
            rows: mockResolvedPapers1
        });

        const results = await fetchTopicPapers("8572", 5, 0);
    ...
    it("Fetches a topic's associated papers using custom pagination", async () => {
        ...
        // LIMIT -> 10, OFFSET -> 40
        const results = await fetchTopicPapers("8572", 10, 40);

        const [query, params] = pool.query.mock.calls[0];

        expectFetchTopicPapersQuery(query);
        expect(params).toEqual(["8572", 10, 40]);
